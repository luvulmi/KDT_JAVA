...
//	=> id를 이용해서 selectOne 한 결과  return
	...
	public List<GroupDTO> groupTest() {
		...
		// => Equ_Join
		// select s.jno, j.jname, count(*), sum(age) , avg(age), max(age), min(age)
		// from jo j, student s where s.jno = j.jno group by jno
		...
		try {
			...
			if (rs.next()) {
				do {
					GroupDTO dto = new GroupDTO(); // DTO 객체 생성
	...
	public void transactionTest() {
		sql="insert into student values('xxx1','엑스맨1',77,5,'테스트',777.77,'1907-07-07')";
		/* 1) Test1
		// => Transaction 적용하지 않고 동일 자료 2회 입력.
		try {
			pst = cn.prepareStatement(sql);
			pst.executeUpdate(); // ok.
			pst.executeUpdate(); // 키 중복으로 인한 오류 발생. -> exception 발생. -> catch 블럭.
		} catch (Exception e) {
		...
		try {
			cn.setAutoCommit(false); // start transaction 쿼리문 의미.
			pst= cn.prepareStatement(sql);
			pst.executeUpdate(); // ok
			pst.executeUpdate(); // exception
			...
			// => Exception 발생
			try {
				cn.rollback(); // rollback 사용시 try-catch문 필요.
